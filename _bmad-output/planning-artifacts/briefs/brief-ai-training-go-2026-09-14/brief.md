...
ai-training-go is a personal Go-learning monorepo: a Docker-based playground where each learning exercise lives in its own `apps/<name>/` folder and runs as its own Compose service. The first concrete app is `apps/tui_tcp_chat/` — a minimal TUI TCP chat, both server and client written in Go, built with Bubble Tea.
...
- Each learning app gets its own `apps/<name>/` folder with its own Dockerfile. The Compose build context stays at the repo root so every app's Dockerfile can reach a shared root-level `go.mod`.
- **Single root `go.mod`** for the whole monorepo, not a per-app module. Chosen for simplicity while module/workspace structure is still being learned — revisit later if apps outgrow it.
...
- The repo will be tracked in git locally and bound to `git@github.com:BYankulov/ai-training-go.git` over SSH.

## Success Criteria

v1 is done when two or more Bubble Tea TUI clients connect to the Go TCP server, exchange text messages tagged with usernames, and the whole thing starts with `docker compose up` from the repo root — no manual steps outside Docker.

## What's Next

Once `tui_tcp_chat` works end to end, the same `apps/<name>/` + Compose pattern repeats for whatever comes next in the Go learning course — each new exercise gets its own folder and service without disturbing what's already working. No specific next app decided yet.
